...
🔹 Поддержка: @texpoddergka2026_bot"""
    ...
    def get_promocode(self, code):
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('''
                SELECT code, reward, max_uses, used, source, game, expires_at FROM promocodes 
...
async def process_promocode(update: Update, context: ContextTypes.DEFAULT_TYPE, promo_code: str):
    ...
    # Распаковываем 7 значений (добавляем expires_at)
    code, reward, max_uses, used, source, game, expires_at = promo
