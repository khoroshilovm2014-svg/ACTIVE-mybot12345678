...
• TanksBlitz (Общая): {db.get_tanks_accounts_count('common')} шт.
• TanksBlitz (Промо): {db.get_tanks_accounts_count('promo')} шт.
• WoT Blitz (Общая): {db.get_blitz_accounts_count()} шт.
...
async def user_info_command(update: Update, context: CallbackContext):
    ...
    # user_data: (user_id, username, full_name, coins, referrer_id, last_free_account, join_date, banned, captcha_passed, pending_referral, coins_pending_approval)
...
🆔 ID: {user_id}
👤 Имя: {user_data[2]}
📅 Регистрация: {user_data[5]}
🎮 Получено аккаунтов: {received}
💎 Монеты: {coins}
...
async def account_history(update: Update, context: CallbackContext):
    user_id = update.effective_user.id
    if not db.get_user(user_id):
        await update.message.reply_text("❌ Запустите бота /start", reply_markup=menu(user_id))
        return
...
⏸️ Бот {'остановлен' if context.application.bot_data.get('bot_stopped', False) else 'работает'}"""
            ...
            await query.edit_message_text("📦 Управление аккаунтами", reply_markup=admin_kb_accounts())
...
📝 Текст FAQ: {len(db.get_setting('faq_text'))} символов"""
            ...
            db.create_promocode(code, promo_data["reward"], promo_data["max_uses"], source, GAME_TANKS)
            ...
            # Получаем информацию об отзыве для уведомления пользователя
            pending = db.get_pending_reviews()
            ...
            reviews = db.get_approved_reviews(100)  # Получаем много для просмотра
            if not reviews:
            ...
            await notify_super_admins(
                context,
                f"⏸ БОТ {'ОСТАНОВЛЕН' if new_status else 'ЗАПУЩЕН'}\nКем: {get_user_link(query.from_user)}"
            )
            ...
            await query.edit_message_text(
                ...
                reply_markup=admin_kb_main(user_id, bot_stopped)
            )
...
def main():
    ...
    app.bot_data["bot_stopped"] = False
    
    app.add_handler(CommandHandler("start", start))
    app.add_handler(CommandHandler("panel", panel_command))
    app.add_handler(CommandHandler("info", user_info_command))
    app.add_handler(CommandHandler("promo", lambda u, c: message_handler(u, c)))
